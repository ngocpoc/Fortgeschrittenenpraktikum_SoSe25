...
Since both glass plates have an initial tilt of $\Theta_0 = \pm\SI{10}{\degree}$, the equation \ref{eqn:delta_glass_vontheta} has to be modified. 
For use in the theoretical model, both angles were converted to radians before substitution into the formula.
When using the relation \ref{eqn:M}, this leads to
\begin{align}
...
$$
    n_{\text{glass, exp}} = \SI{1.52\pm0.22}\,.
$$
In this evaluation, the first two fringe counts were excluded from the average, as they deviated significantly form the remaining values. These values likely represent early misalignments or transient instability in the setup before the system stabilized. Their exclusion ensures a more reliable and consistent estimation of the refractive index.
\begin{table}[h]
    \centering
    \caption{Measured numbers $M$ of interference maxima while rotating a glass plate by $\Theta=\SI{10\pm2}{\degree}$ and the resulting arithmetic mean $\bar{M}$ (excluding the first two values).}
    \label{tab:glass_counts}
    \begin{tblr}{colspec= c|c c c c c c c c c c || c}
        \toprule
        $M$ & 28 & 27 & 33 & 33 & 33 & 33 & 33 & 33 & 32 & 32 & $\bar{M} = 32.8\pm0.4$\\
        \bottomrule
    \end{tblr}
\end{table}
